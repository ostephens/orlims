			...
			if(data){
				if(data.error > 0) {
					...
					holding_html_open = '' +'<div class="availabilityHeader" style="color:#002147">' +
							'<div class="availabilityTitle"><strong>Availability</strong></div>'+
							'<div class="availabilityNav"><a href="#" onclick="hideAll(\''+oxfalephId+'\')">Hide all Libraries</a>';
					...
					holding_html_open += ' / <a href="#" onclick="showAll(\''+oxfalephId+'\')">Show all Libraries</a></div>' +
							...
							'<div class="availabilityTable"><table class="oxfaleph_summary" id="'+oxfalephId+'"><thead><tr><th>Library' +
					...
					$jq.each(data, function(index, oxfaleph_doc) {
						var library_info = '';
						if(libWeb[oxfaleph_doc.libcode]) {
							library_info = '<a href="'+libWeb[oxfaleph_doc.libcode]+'">i<i class="fa fa-info-circle"></i></a>';
						}
						holdings_html += '<tr class="'+oxfaleph_doc.libcode+' collapsed'+'"><td>'+oxfaleph_doc.libname+'</td>'+
										 ...
										 '<td>'+library_info+'</td></tr>'; //need to get URL into this row
						}
					);
				}
			}
